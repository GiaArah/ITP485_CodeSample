...
void Graphics::InitD3D(HWND hWnd, float width, float height)
{
    ...
    { // Set up Depth/Stencil State for Z-Buffer
        // Use the Less or Equal comparison
        ID3D11DepthStencilState* pDSState = CreateDepthStencilState(D3D11_COMPARISON_FUNC::D3D11_COMPARISON_LESS_EQUAL);
        GetDeviceContext()->OMSetDepthStencilState(pDSState, 0);

        // Release depth state as we no longer need it once it's set
        pDSState->Release();
    }

    { // Set up depth stencil view
        CreateDepthStencil(static_cast<int>(mScreenWidth), static_cast<int>(mScreenHeight), &mDepthTexture, &mDepthStencilView);
    }
    ...
    { // Initialize sampler state
        ...
        // Use description to initialize the sampler state
        GetDevice()->CreateSamplerState(&sampDesc, &mSamplerState);
    ...
    { // change the forward-facing winding order through raster states
        ...
        GetDevice()->CreateRasterizerState(&rasterDesc, &rasterState);
        GetDeviceContext()->RSSetState(rasterState);
        rasterState->Release();
    }
    ...
    // we'll be drawing triangle lists
...
ID3D11DepthStencilState* Graphics::CreateDepthStencilState(D3D11_COMPARISON_FUNC inDepthComparisonFunction)
{
    ...
    */
    ...
    GetDevice()->CreateDepthStencilState(&dsDesc, &pDSState);
    return pDSState;
}
...
bool Graphics::CreateDepthStencil(int inWidth, int inHeight, ID3D11Texture2D** pDepthTexture, ID3D11DepthStencilView** pDepthView)
{
    ...
    */
    ...
    // Create 2D texture
    GetDevice()->CreateTexture2D(&descDepth, NULL, pDepthTexture);

    // Set Depth Stencil View Parameters
    D3D11_DEPTH_STENCIL_VIEW_DESC pStencilViewDesc = {};
    pStencilViewDesc.Format = DXGI_FORMAT_D24_UNORM_S8_UINT;
    pStencilViewDesc.ViewDimension = D3D11_DSV_DIMENSION_TEXTURE2D;
    pStencilViewDesc.Texture2D.MipSlice = 0;
    pStencilViewDesc.Flags = 0;

    // Create Depth Stencil View
    int result = GetDevice()->CreateDepthStencilView(*pDepthTexture, &pStencilViewDesc, pDepthView);
    if (result == 0)
    {
        return true;
    }
...
ID3D11Buffer* Graphics::CreateGraphicsBuffer(const void* initialData, int inDataSize, D3D11_BIND_FLAG inBindFlags, D3D11_CPU_ACCESS_FLAG inCPUAccessFlags, D3D11_USAGE inUsage)
{
    ...
    char output[1024];
    sprintf_s(output, 1024, "0x%016llx\n", (uintptr_t)buffer);
...
{
    GetDeviceContext()->ClearDepthStencilView(GetDepthStencilView(), D3D11_CLEAR_DEPTH, depth, 0);
}
